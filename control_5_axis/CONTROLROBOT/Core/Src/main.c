...
bool flag_receive_uart = false, flag_done_base = false, flag_done_a1 = false,
		flag_done_a2 = false, flag_done_a3 = false, flag_done_a4 = false,
		flag_dir_change_base = false, flag_dir_change_a1 = false,
		flag_dir_change_a2 = 0, flag_dir_change_a3 = 0, flag_dir_change_a4 = 0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM3 && flag_receive_uart == true) {
		pulse_a1 = flag_done_a1 == false ? pulse_a1 + 1 : pulse_a1;
		pulse_a3 = flag_done_a3 == false ? pulse_a3 + 1 : pulse_a3;
		if (pulse_a1 >= step_a1 && flag_done_a1 == false) {
			HAL_TIM_PWM_Stop(&htim3, A1_CHANNEL);
			flag_done_a1 = true;
		}
		if (pulse_a3 >= step_a3 && flag_done_a3 == false) {
	...
	if (htim->Instance == TIM2 && flag_receive_uart == true) {
		pulse_a4 = flag_done_a4 == false ? pulse_a4 + 1 : pulse_a4;
		pulse_base = flag_done_base == false ? pulse_base + 1 : pulse_base;
		if (pulse_a4 >= step_a4 && flag_done_a4 == false) {
			HAL_TIM_PWM_Stop(&htim2, A4_CHANNEL);
			flag_done_a4 = true;

		}
		if (pulse_base >= step_base && flag_done_base == false) {
			HAL_TIM_PWM_Stop(&htim2, A1_CHANNEL);
			flag_done_base = true;
		}
	}
	if (htim->Instance == TIM4 && flag_receive_uart == true) {
		pulse_a2 = flag_done_a2 == false ? pulse_a2 + 1 : pulse_a2;
		if (pulse_a2 >= step_a2 && flag_done_a2 == false) {
	...
	if ((flag_done_base == true && flag_done_a1 == true && flag_done_a2 == true
			&& flag_done_a3 == true && flag_done_a4 == true
			&& flag_receive_uart == true)) {
		count_finish_timer = 0;
	...
	if (huart->Instance == USART1) {
		...
		for (int i = 0; i < length; i++) {
			if ((rxByte[i] >= 'a' && rxByte[i] <= 'z')
				...
				case 0:
					if (rxByte[i] == '-') {
						HAL_GPIO_WritePin(GPIOA, DIR_BASE_Pin, CounterClockWise);
...
//								CounterClockWise);
				...
				case 1:
					if (rxByte[i] == '-') {
						HAL_GPIO_WritePin(GPIOA, DIR_A1_Pin,
								CounterClockWise);
				...
				case 2:
					if (rxByte[i] == '-') {
						HAL_GPIO_WritePin(GPIOA, DIR_A2_Pin,
								CounterClockWise);
						value_a2 /= 10;
				...
				case 3:
					if (rxByte[i] == '-') {
						HAL_GPIO_WritePin(GPIOA, DIR_A3_Pin,
								CounterClockWise);
						value_a3 /= 10;
				...
				case 4:
					if (rxByte[i] == '-') {
						HAL_GPIO_WritePin(GPIOA, DIR_A4_Pin,
								CounterClockWise);
						value_a4 /= 10;
			...
			flag_done_base = false;
			flag_done_a1 = false;
			flag_done_a2 = false;
			flag_done_a3 = false;
			flag_done_a4 = false;
			if (step_base) {
				HAL_TIM_PWM_Start(&htim2, A1_CHANNEL);
			} else {
				flag_done_base = true;
			}
			if (step_a1) {
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOA, DIR_A4_Pin|DIR_BASE_Pin|DIR_A1_Pin|DIR_A2_Pin
                          |DIR_A3_Pin, GPIO_PIN_RESET);
