...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM2 && flag_receive_uart == true) {
			pulse_a4 = !(flag_finish_timer & (1<<BIT_STOP_A4)) ? pulse_a4 + 1 : pulse_a4;
			pulse_base = !(flag_finish_timer & (1<<BIT_STOP_BASE)) ? pulse_base + 1 : pulse_base;
			if (pulse_a4 >= step_a4) {
	...
	if (htim->Instance == TIM3 && flag_receive_uart == true) {
		pulse_a1 = !(flag_finish_timer & (1<<BIT_STOP_A1)) ? pulse_a1 + 1 : pulse_a1;
		pulse_a3 = !(flag_finish_timer & (1<<BIT_STOP_A3)) ? pulse_a3 + 1 : pulse_a3;
		if (pulse_a1 >= step_a1) {
	...
	if (htim->Instance == TIM4 && flag_receive_uart == true) {
		pulse_a2 = (flag_finish_timer & (1<<BIT_STOP_A2)) ? pulse_a2 + 1 : pulse_a2;
		if (pulse_a2 >= step_a2) {
